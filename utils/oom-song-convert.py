...
def help():
		print "Utility to convert OOMidi 0.6 songs to 0.7 or newer song format."
		print "Usage: oom-convert <song file to convert>"
		print ""
...
print "OOMidi Song converter %s"%version
print ""
...
convert = OOMidiConvert()

#take care of first few lines:
outFile.write(inFile.readline()) # <?xml
line = inFile.readline() # <oom string
if string.find(line, "<oom version=\"1.0\">") == -1:
		print "Unable to confirm that this is a song file with the old format, aborting..."
		sys.exit("")
outFile.write("<oom version=\"2.0\" comment=\"converted by oom converter v%s\">\n"%version)
