...
%define oomver  			0.7.0pre3
%define oomdir  			0.7.0pre3
%define capver   			1.0
...
Name:				%{name}
Summary:			OOMidi is a MIDI/Audio sequencer with recording and editing capabilities.
Version:			%{oomversion}
Release:			%{release}
URL:				http://oom.sourceforge.net/
Source0:			%{name}-%{oomver}.tar.bz2
...
%description
OOMidi is a MIDI/Audio sequencer with recording and editing capabilities.
Some Highlights:
...
?package(%{name}): longtitle="OOMidi audio editor" \
command="/usr/bin/oom" title="OOMidi audio editor"  needs="x11" \
...
- Added oom-softsynth-fix
  on oom 0.6.1, it results in a dead lock when oom starts a softsynth
