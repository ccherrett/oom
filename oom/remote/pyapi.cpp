...
//------------------------------------------------------------
// addPyPartEventsToOOMidiPart
//  parse events from python part structure into oom part
//------------------------------------------------------------

bool addPyPartEventsToOOMidiPart(MidiPart* npart, PyObject* part)
{
	PyObject* events;
	...
	//
	// Go through event list, create OOMidi events of them and add to new part
	//
 ...
 */
//------------------------------------------------------------
// Global method definitions for OOMidi:s Python API
//
...
{
	Py_Initialize();
	PyImport_AddModule("oom");
	Py_InitModule("oom", g_methodDefinitions);
	...
	string launcherfilename = string(SHAREDIR) + string("/pybridge/oomplauncher.py");
	printf("Initiating OOMidi Pybridge launcher from %s\n", launcherfilename.c_str());
