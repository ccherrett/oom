...
SndFile* getWave(const QString& inName, bool readOnlyFlag)
{
	...
	{
		name = oomProject + QString("/") + name;
	}
	else
	{
		if (!QFile::exists(name))
		{
			if (QFile::exists(oomProject + QString("/") + name))
			{
				name = oomProject + QString("/") + name;
			}
		}
	...
	if (f == 0)
	{
		...
		if (error)
		{
			...
			QMessageBox::critical(NULL, "OOMidi import error.",
					"OOMidi failed to import the file.\n"
					"Possibly this wasn't a sound file?\n"
					"If it was check the permissions, OOMidi\n"
...
bool OOMidi::importWaveToTrack(QString& name, unsigned tick, Track* track)
{
	...
	/*if ((unsigned) sampleRate != f->samplerate())
	{
		if (QMessageBox::question(this, tr("Import Audio file"),
				...
				QString::null, 0, 1))
		{
			//printf("why won't oom let me delete the file object? %d\n", f->getRefCount());
			if (f->getRefCount() == 0)
				delete f;
			return true;
		}
