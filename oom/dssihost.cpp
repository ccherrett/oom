...
int DssiSynthIF::oscUpdate(lo_arg **argv)
	  ...
	  #ifdef DSSI_DEBUG
	  printf("DssiSynthIF::oscUpdate synth name:%s url:%s uiTarget:%p uiOscPath:%s uiOscConfigurePath:%s oomProject:%s\n", synti->name().ascii(), url, uiTarget, uiOscPath, uiOscConfigurePath, oomProject.ascii());
	  #endif
      
	  //lo_send(uiTarget, uiOscConfigurePath, "ss",
		 //DSSI_PROJECT_DIRECTORY_KEY, song->projectPath().toAscii().data());
	  lo_send(uiTarget, uiOscConfigurePath, "ss",
		 DSSI_PROJECT_DIRECTORY_KEY, oomProject.ascii());
...
bool DssiSynthIF::init(DssiSynth* s)
{
	...
	if (dssi->configure)
	{
		char *rv = dssi->configure(handle, DSSI_PROJECT_DIRECTORY_KEY,
				oomProject.toLatin1().constData()); //song->projectPath()

		if (rv)
		{
			fprintf(stderr, "OOMidi: Warning: plugin doesn't like project directory: \"%s\"\n", rv);
			free(rv);
		}
...
int DssiSynthIF::oscControl(unsigned long port, float value)
{
	...
	// TODO: Now OOMidi should forget about all the VST fifo events past ten+ (?) seconds. Add event timestamps...
	...
	{
		// TODO: TODO: Update midi OOMidi's midi controller knobs, sliders, boxes etc with a call to the midi port's setHwCtrlState() etc.
...
int DssiSynthIF::oscConfigure(const char *key, const char *value)
{
	...
	{
		fprintf(stderr, "OOMidi: OSC: UI for plugin '%s' attempted to use reserved configure key \"%s\", ignoring\n",
	...
	if (message)
	{
		printf("OOMidi: on configure '%s' '%s', plugin '%s' returned error '%s'\n",
...
int DssiSynthIF::pluginID()
{
	return (synth && synth->dssi) ? synth->dssi->LADSPA_Plugin->UniqueID : 0;
}

int DssiSynthIF::id()
{
	return 0;
} // Synth is not part of a rack plugin chain. Always 0.

QString DssiSynthIF::pluginLabel() const
{
	return (synth && synth->dssi) ? QString(synth->dssi->LADSPA_Plugin->Label) : QString();
}

QString DssiSynthIF::name() const
{
	return synti->name();
}

AudioTrack* DssiSynthIF::track()
{
	return (AudioTrack*) synti;
}

void DssiSynthIF::enableController(int i, bool v)
{
	controls[i].enCtrl = v;
}

bool DssiSynthIF::controllerEnabled(int i) const
{
	return controls[i].enCtrl;
}

bool DssiSynthIF::controllerEnabled2(int i) const
{
...
bool DssiSynthIF::readConfiguration(Xml& /*xml*/, bool /*readPreset*/)
{
	return false;
}

int DssiSynthIF::parameters() const
{
	return synth ? synth->_controlInPorts : 0;
}

void DssiSynthIF::setParam(int i, double val)
{
	setParameter(i, val);
}

double DssiSynthIF::param(int i) const
{
	return getParameter(i);
}

const char* DssiSynthIF::paramName(int i)
{
	return (synth && synth->dssi) ? synth->dssi->LADSPA_Plugin->PortNames[i] : 0;
}
//LADSPA_PortRangeHint DssiSynthIF::range(int i)               { return (synth && synth->dssi) ? synth->dssi->LADSPA_Plugin->PortRangeHints[i] : 0; }

LADSPA_PortRangeHint DssiSynthIF::range(int i)
{
	return synth->dssi->LADSPA_Plugin->PortRangeHints[i];
}
