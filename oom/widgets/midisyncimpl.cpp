...
enum
{
	DEVCOL_NO = 0, DEVCOL_NAME, DEVCOL_IN, DEVCOL_TICKIN, DEVCOL_MRTIN, DEVCOL_MMCIN, DEVCOL_MTCIN, DEVCOL_MTCTYPE,
...
void MidiSyncConfig::setWhatsThis(QTreeWidgetItem *item)
{
	...
	item->setWhatsThis(DEVCOL_TCLK, tr("Send midi clock output. If 'Slave to External Sync' is chosen,\n"
			" oom can re-transmit clock to any other chosen ports."));
...
void MidiSyncConfig::dlvDoubleClicked(QTreeWidgetItem* item, int col)
{
	...
	{
		int val = lvi->_idIn;
		int newval = QInputDialog::getInteger(this, "OOMidi: Sync info", "Enter new id number (127 = all):", val, 0, 127, 1, &ok);
		if (ok)
		{
			lvi->_idIn = newval;
	...
	else if (col == DEVCOL_TID)
	{
		int val = lvi->_idOut;
		int newval = QInputDialog::getInteger(this, "OOMidi: Sync info", "Enter new id number (127 = global):", val, 0, 127, 1, &ok);
		if (ok)
		{
			lvi->_idOut = newval;
