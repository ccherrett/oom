...
QString MidiAlsaDevice::open()
{
	...
	if (_openFlags & 1)
	{
		snd_seq_port_subscribe_set_sender(subs, &oomPort);
	...
	if (_openFlags & 2)
	{
		snd_seq_port_subscribe_set_dest(subs, &oomPort);
...
bool initMidiAlsa()
{
	...
	int port = snd_seq_create_simple_port(alsaSeq, "OOMidi Port 0",
	...
	oomPort.port = port;
	oomPort.client = snd_seq_client_id(alsaSeq);
...
void alsaScanMidiPorts()
{
	...
	while (snd_seq_query_next_client(alsaSeq, cinfo) >= 0)
	{
		...
		while (snd_seq_query_next_port(alsaSeq, pinfo) >= 0)
		{
			...
			if (adr.client == oomPort.client && adr.port == oomPort.port)
				continue;
			int flags = 0;
			if (capability & outCap)
				flags |= 1;
	...
	for (;;)
	{
		...
		switch (ev->type)
		{
			...
			case SND_SEQ_EVENT_SYSEX:
				...
				{
					printf("OOMidi: alsaProcessMidiInput sysex chunks not supported!\n");
					break;
				}
