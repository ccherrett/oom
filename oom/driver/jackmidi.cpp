...
//extern oom_jack_midi_buffer jack_midi_out_data[JACK_MIDI_CHANNELS];
//extern oom_jack_midi_buffer jack_midi_in_data[JACK_MIDI_CHANNELS];
...
iJackMidiPort JackMidiPortList::createClientPort(int flags) // 1 = writable, 2 = readable - do not mix
{
  ...
  {
	char buf[80];
	snprintf(buf, 80, "oom-jack-midi-out-%d", _nextOutIdNum);
  ...
  {  
	char buf[80];
	snprintf(buf, 80, "oom-jack-midi-in-%d", _nextInIdNum);
	...
	/*
	{
	  if(audioDevice->deviceType() == AudioDevice::JACK_AUDIO)       // p3.3.52
	  {
		...
		if(!out_client_jackport)   // p3.3.55
		{
		  ...
		  fprintf(stderr, "OOMidi: createJackMidiDevice failed creating output port name %s\n", (name + QString(JACK_MIDI_OUT_PORT_SUFFIX)).toLatin1().constData()); // p3.3.55
	...
	/*
	{
	  if(audioDevice->deviceType() == AudioDevice::JACK_AUDIO)       // p3.3.52
	  {
		...
		if(!in_client_jackport)    // p3.3.55
		{
		  ...
		  fprintf(stderr, "OOMidi: createJackMidiDevice failed creating input port name %s\n", (name + QString(JACK_MIDI_IN_PORT_SUFFIX)).toLatin1().constData());
...
bool MidiJackDevice::putMidiEvent(const MidiPlayEvent& /*event*/)
{
	...
	if(jack_midi_out_data[channel].buffer[give*4+3]){
	  fprintf(stderr, "WARNING: oom-to-jack midi-buffer is full, channel=%u\n", channel);
	  return false;
	}
	...
	switch(event.type()){
	  ...
	  case ME_PITCHBEND:
		...
		// convert oom pitch-bend to midi standard
...
void MidiJackDevice::processEvent(const MidiPlayEvent& event)
{
		...
		if (event.type() == ME_CONTROLLER)
	{
		...
		{ // 7 Bit Controller
			queueEvent(event);
			//queueEvent(oomport, MidiPlayEvent(t, port, chn, event));
		}
